module FakeLogin
  def fake_login user = User.new(:user_name => 'fakeuser', :role_names => ["default"])
    ...
    Role.stub!(:by_name).with(:key => "default").and_return(Role.new(:name => "default", :permissions => [Permission::REGISTER_CHILD]))
  ...
  def fake_field_worker_login
    ...
    user.stub!(:roles).and_return([Role.new(:permissions => [Permission::REGISTER_CHILD])])
    fake_login user
  end
end
