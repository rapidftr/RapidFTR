...
describe 'shared/_form_section.html.erb', :type => :view do
  ...
  describe 'translating form section name' do
    it 'should be shown with translated name' do
      ...
      render :partial => 'shared/tabs', :object => [@form_section], :formats => [:html], :handlers => [:erb]
  ...
  describe 'translating form section heading' do
    it 'should be shown with translated heading' do
      ...
      render :partial => 'shared/show_form_section', :formats => [:html], :handlers => [:erb]
  ...
  describe 'rendering text fields' do
    ...
    context 'new record' do
      ...
      it 'renders text fields with a corresponding label' do
        ...
        @child = Child.new
        render :partial => 'shared/form_section', :locals => {:form_section => @form_section, :model => @child}, :formats => [:html], :handlers => [:erb]

        @form_section.fields.each do |section_field|
          expect(rendered).to be_include("<label class=\"key\" for=\"#{section_field.tag_id @child}\">")
          expect(rendered).to be_include("<input id=\"#{section_field.tag_id @child}\" name=\"#{section_field.tag_name_attribute(@child)}\" type=\"text\" />")
        end
      end
    end
