...
describe SearchController, :type => :controller do
  ...
  describe 'GET search for children' do
    let :search do
  ...
  describe 'GET search for enquiries' do
    let :search do
      search = instance_double('Search', :results => [])
      allow(search).to receive(:created_by).and_return(search)
      allow(search).to receive(:fulltext_by).and_return(search)
    ...
    it 'should search against specified model' do
      expect(Search).to receive(:for).with(Enquiry).and_return(search)

      get :search, :query => 'test', :search_type => 'Enquiry'
      expect(response).to be_ok
    end
    ...
    it 'should authorize the user' do
      expect(@controller).to receive(:authorize!).with(:index, Enquiry)
