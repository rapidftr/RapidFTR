...
describe AdvancedSearchController do
  ...
  it "should create advanced user criteria for created by" do
    ...
    SearchCriteria.should_receive(:create_advanced_criteria).with( hash_including(:field => "created_by") )
  ...
  it "should create advanced user criteria for updated by" do
    ...
    SearchCriteria.should_receive(:create_advanced_criteria).with( hash_including(:field => "last_updated_by") )
  ...
  it "should filter search results by date created" do
    ...
    SearchService.should_receive(:filter_by_date).with("child_records", created_at_start, created_at_end, :created_at)
  ...
  it "should filter search results by date updated" do
    SearchCriteria.stub(:build_from_params).and_return(["criteria_list"])
    SearchCriteria.stub(:create_advanced_criteria).and_return("advanced_criteria")
    SearchService.stub(:search).and_return("child_records")

    last_updated_at_start = "30-01-2011"
    last_updated_at_end  = "20-02-2011"

    SearchService.should_receive(:search)
    SearchService.should_receive(:filter_by_date).with("child_records", last_updated_at_start, last_updated_at_end, :last_updated_at)
    get :index, :criteria_list => {"0"=>{"field"=>"name_of_child", "value"=>"joe joe", "index"=>"0"}}, 
      :last_updated_at_start_value => last_updated_at_start, :last_updated_at_end_value => last_updated_at_end
  ...
  it "should construct criteria list for only advanced user criteria" do
    ...
    SearchCriteria.stub(:new).and_return("empty_criteria")
    SearchService.stub(:search).and_return([])
    SearchService.should_receive(:search).with(["advanced_user_details"])
