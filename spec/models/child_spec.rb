...
describe Child, :type => :model do
  ...
  describe ".search" do
    ...
    it "should return empty array if search is not valid" do
      ...
      expect(Child.search(search)).to eq([])
    end
    ...
    it "should return a fuzzy match" do
      ...
      expect(Child.search(search).first.map(&:name)).to match_array(["timithy", "timothy"])
    end
    ...
    it "should return children that have duplicate as nil" do
      ...
      expect(result.first.map(&:name)).to eq(["eduardo aquiles"])
    end
    ...
    it "should search by exact match for short id" do
      ...
      expect(results.first[:unique_identifier]).to eq("0987654321")
    end
    ...
    it "should match more than one word" do
      ...
      expect(Child.search(search).first.map(&:name)).to match_array(["timothy cochran"])
    end
    ...
    it "should return the children registered by the user if the user has limited permission" do
      ...
      expect(Child.search_by_created_user(search, "you", 1).first.map(&:name)).to match_array(["kavitha"])
    end
  ...
  describe ".sunspot_search" do
    ...
    it "should return all results" do
      ...
      expect(Child.sunspot_search(1, query).last.count).to eq(40)
    end
  end
