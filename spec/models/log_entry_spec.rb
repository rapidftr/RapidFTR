...
describe LogEntry, :type => :model do
  ...
  it 'should return all entries sorted by created_at date' do
    ...
    LogEntry.create!(:type => LogEntry::TYPE[:csv], :username => 'rapidftr', :organisation => 'urc', :export_format => 'cpims', :number_or_records => '123')
    allow(Clock).to receive(:now).and_return(1.day.from_now)
    LogEntry.create!(:type => LogEntry::TYPE[:cpims], :username => 'rapidftr', :organisation => 'urc', :export_format => 'cpims', :number_or_records => '123')
    allow(Clock).to receive(:now).and_return(2.days.ago)
    LogEntry.create!(:type => LogEntry::TYPE[:pdf], :username => 'rapidftr', :organisation => 'urc', :export_format => 'cpims', :number_or_records => '123')
