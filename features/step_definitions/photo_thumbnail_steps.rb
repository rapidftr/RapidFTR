...
Then /^I should see the primary photo thumbnail of "([^\"]*)"$/ do |child_name|
  check_link(response, child_name) {|child| child_thumbnail_path(child, child.primary_photo_id)}
end

Then /^I should not see the primary photo thumbnail of "([^\"]*)"$/ do |child_name|
  check_no_link(response, child_name) {|child| child_thumbnail_path(child, child.primary_photo_id)}
end
