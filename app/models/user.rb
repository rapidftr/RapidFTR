...
class User < CouchRestRails::Document
  ...
  validates_presence_of :full_name, :message => I18n.t("activerecord.errors.models.user.full_name")
  validates_presence_of :password_confirmation, :message => I18n.t("activerecord.errors.models.user.password_confirmation"), :if => :password_required?
  validates_presence_of :role_ids, :message => I18n.t("activerecord.errors.models.user.role_ids"), :if => Proc.new {|user| user.verified}
  validates_presence_of :organisation, :message => I18n.t("activerecord.errors.models.user.organisation")

  validates_format_of :user_name, :with => /^[^ ]+$/, :message => I18n.t("activerecord.errors.models.user.user_name")

  validates_format_of :email, :with => /^([^@\s]+)@((?:[-a-zA-Z0-9]+\.)+[a-zA-Z]{2,})$/, :if => :email_entered?,
                      :message => I18n.t("activerecord.errors.models.user.email")
  ...
  def is_user_name_unique
    ...
    [false, I18n.t("activerecord.errors.models.user.user_name_uniqueness")]
  end

  def authenticate(check)
    if new?
      raise Exception.new, I18n.t("activerecord.errors.models.user.authenticate")
    end
