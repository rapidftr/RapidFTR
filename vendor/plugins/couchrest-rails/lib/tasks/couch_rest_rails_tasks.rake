namespace :couchdb do
  ...
  task :create, [ :database ] => :environment do |t, args|
  ...
  task :delete, [ :database ] => :environment do |t, args|
    args.with_defaults(:database => "*", :opts => {})
  ...
  namespace :fixtures do
    ...
    task :load, [ :database ] => :environment do |t, args|
      args.with_defaults(:database => "*")
      puts CouchRestRails::Fixtures.load(args.database)
    end
    task :dump, [ :database ] => :environment do |t, args|
      args.with_defaults(:database => "*")
  ...
  namespace :views do
    ...
    task :push, [ :database ] => :environment do |t, args|
      args.with_defaults(:database => "*", :opts => {})
